...
#' Coefficient of determination, computed the standard way:
#' 1 - SSE/SST with SST measured on the OBSERVED values.
r2_score <- function(observed, predicted) {
  1 - sum((observed - predicted)^2) / sum((observed - mean(observed))^2)
}
rmse_score <- function(observed, predicted) sqrt(mean((observed - predicted)^2))

#' Load the pre-generated second-dimension variable sets named in the config.
load_reference_vars <- function(which = REFERENCE_VARS) {
  out <- lapply(which, function(v) {
    p <- file.path(PROJ_ROOT, REFERENCE_DIR, paste0("sda-", tolower(v), ".csv"))
    if (!file.exists(p)) stop("missing reference variable set: ", p)
